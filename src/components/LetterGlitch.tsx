...
const BRAND_VARS = ['--color-heading1-light', '--theme-background', '--theme-accent', '--color-accent-dark'];
...
const LetterGlitch = ({
  ...
  const drawLetters = () => {
    ...
    const themeFont = getComputedStyle(document.documentElement).getPropertyValue('--theme-font').trim() || 'monospace';
    ctx.font = `${fontSize}px "${themeFont}"`;
  ...
  return (
    <div className="relative w-full h-36 bg-background overflow-hidden">
      <canvas ref={canvasRef} className="block w-full h-full" />
      {outerVignette && (
        <div 
          className="absolute top-0 left-0 w-full h-full pointer-events-none"
          style={{
            background: 'radial-gradient(circle, transparent 60%, var(--theme-background) 100%)'
          }}
        />
      )}
      {centerVignette && (
